...
var (
	...
	systemContracts = map[common.Address]bool{
		...
		common.HexToAddress(systemcontracts.TransferController):         true,
	}
)
...
func (p *Parlia) distributeIncoming(val common.Address, state *state.StateDB, header *types.Header, chain core.ChainContext,
	...
	doDistributeSysReward := !p.chainConfig.IsKepler(header.Number, header.Time) &&
		state.GetBalance(common.HexToAddress(systemcontracts.ValidatorContract)).Cmp(maxSystemBalance) < 0
...
func (p *Parlia) initContract(state *state.StateDB, header *types.Header, chain core.ChainContext,
	...
	contracts := []string{
		...
		systemcontracts.TransferController,
	}
	// get packed data
	data, err := p.validatorSetABI.Pack(method)
	if err != nil {
...
func (p *Parlia) distributeToSystem(amount *big.Int, state *state.StateDB, header *types.Header, chain core.ChainContext,
	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) error {
	// get system message
	msg := p.getSystemMessage(header.Coinbase, common.HexToAddress(systemcontracts.ValidatorContract), nil, amount)
	// apply message
	return p.applyTransaction(msg, state, header, chain, txs, receipts, receivedTxs, usedGas, mining)
}
